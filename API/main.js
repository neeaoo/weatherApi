...
function searchWeather(searchTerm) {
    ...
    fetch(`http://api.openweathermap.org/data/2.5/weather?id=myrtsi&lang=fi&${searchMethod}=${searchTerm}&APPID=${appID}&units=${units}`).then(result => {
        return result.json();
    }).then(result => {
        init(result);
    })
...
function init(resultFromServer) {
    ...
    windSpeedElement.innerHTML ='Tuulen noupeus ' + Math.floor(resultFromServer.wind.speed) + ' m/s';
